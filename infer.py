...
from dataset import FeatureSchema, PCVRParquetDataset, TIME_SCALE_CONFIG
...
# saved state_dict.
#
# Special note on ``time_scale_config``: this value is strictly determined by
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Special handling for ``time_scale_config``: it is not exposed on the CLI
    as an independent hyperparameter; the config is uniquely determined by
    ``dataset.TIME_SCALE_CONFIG``. Resolution order:

      1) ``train_config`` contains ``time_scale_config`` directly
         -> use that value;
      2) ``train_config`` contains ``use_time_buckets`` (boolean flag)
         -> derive as ``TIME_SCALE_CONFIG`` or ``None``;
      3) neither is present -> fall back to ``_FALLBACK_MODEL_CFG[...]``.
    """
    cfg: Dict[str, Any] = {}
    for key in _MODEL_CFG_KEYS:
        if key == 'time_scale_config':
            if 'time_scale_config' in train_config:
                cfg[key] = train_config['time_scale_config']
            elif 'use_time_buckets' in train_config:
                cfg[key] = TIME_SCALE_CONFIG if train_config['use_time_buckets'] else None
            else:
                cfg[key] = _FALLBACK_MODEL_CFG[key]
                logging.warning(
                    f"train_config missing both 'time_scale_config' and 'use_time_buckets', "
                    f"using fallback = {cfg[key] is not None}")
            continue
...
def build_model(
    ...
    time_cfg = model_cfg.get('time_scale_config', None)
    logging.info(f"Building model - time_scale_config present: {time_cfg is not None}, "
                 f"scales: {list(time_cfg.keys()) if time_cfg else 'disabled'}")
    model = PCVRHyFormer(
...
def log_time_bucket_debug_info(model: nn.Module, state_dict: Dict[str, Any], source: str) -> None:
    """Log debugging information about time bucket related parameters.

    Args:
        model: The current model instance.
        state_dict: The checkpoint state_dict being loaded.
        source: A string describing the source (e.g., 'checkpoint' or 'built_model').
    """
    logging.info(f"=== Time bucket debug info ({source}) ===")

    # Log model's time_embs status
    has_time_embs = getattr(model, 'time_embs', None) is not None
    logging.info(f"Model time_embs: {'enabled' if has_time_embs else 'disabled'}")
    if has_time_embs:
        for scale, emb in model.time_embs.items():
            logging.info(f"  scale '{scale}': Embedding({emb.num_embeddings}, {emb.embedding_dim})")

    # Check for time_embed/time_embs weights in model
    for name, param in model.named_parameters():
        if 'time_emb' in name.lower() or 'time_embs' in name.lower():
            logging.info(f"Model parameter '{name}' shape: {param.shape}")

    # Find all keys in state_dict containing 'time_embs' or 'time_embed' patterns
    for key, tensor in state_dict.items():
        if 'time_emb' in key.lower() or 'time_bucket' in key.lower():
            logging.info(f"Checkpoint tensor '{key}': shape={tensor.shape}, dtype={tensor.dtype}")

    if not any('time_emb' in k.lower() or 'time_bucket' in k.lower() for k in state_dict):
        logging.warning("No time-embedding keys found in checkpoint state_dict")

    # Compare model parameters vs checkpoint tensors for time-related params
    model_time_params = {}
    for name, param in model.named_parameters():
        if 'time_emb' in name.lower():
            model_time_params[name] = param.shape

    for key, tensor in state_dict.items():
        if 'time_emb' not in key.lower() and 'time_bucket' not in key.lower():
            continue
        for model_name, model_shape in model_time_params.items():
            if key == model_name or key.split('.')[-1] == model_name.split('.')[-1]:
                if model_shape != tuple(tensor.shape):
                    logging.error(
                        f"MISMATCH: Key '{key}' - Model shape {model_shape} vs "
                        f"Checkpoint shape {tuple(tensor.shape)}"
                    )

    logging.info(f"=== End time bucket debug info ({source}) ===\n")
...
def load_model_state_strict(
    ...
    log_time_bucket_debug_info(model, state_dict, "checkpoint")
    log_time_bucket_debug_info(model, {}, "built_model")
    
    try:
        model.load_state_dict(state_dict, strict=True)
    except RuntimeError as e:
        error_msg = str(e)
        logging.error(f"RuntimeError during load_state_dict: {error_msg}")
        
        # Parse and report dimension mismatches for time_bucket related params
        import re
        dim_match = re.search(r"size mismatch for (.*)?:\s+copying from a tensor with size (.*) to a tensor of size (.*)",
                              error_msg, re.IGNORECASE)
        if dim_match:
            param_name = dim_match.group(1).strip()
            checkpoint_size = dim_match.group(2)
            model_size = dim_match.group(3)
            
            is_time_related = 'time_embed' in param_name.lower() or 'time_bucket' in param_name.lower()
            if is_time_related:
                logging.error(f"TIME-BUCKET DIM MISMATCH:")
                logging.error(f"  Parameter: {param_name}")
                logging.error(f"  Checkpoint size: {checkpoint_size}")
                logging.error(f"  Model size: {model_size}")
        else:
            # General error parsing
            size_mismatch_parts = re.findall(r'size mismatch for [^:]+: copy[^,]+(?:,\s*expect[^]]+\])?', 
                                             error_msg, re.IGNORECASE)
            for part in size_mismatch_parts[:5]:  # Limit to first 5
                logging.error(f"Dimension issue: {part}")
